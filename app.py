...
def answer_question(course, question, include_web=True, fmt="essay", max_out=8000,
    ...
    if mode == "cases":
        ...
        system = (CONFIG["system_prompt"] + "\n\n" + LEGAL_METHOD + "\n\n"
                  ...
                  + CITATION_INTEGRITY + "\n\n" + PRIMARY_FIRST + "\n\n"
                  + PRECISION_DISCIPLINE + "\n\n" + TEMPORAL_SUCCESSION + "\n\n"
                  "FOCUSED ISSUE ANSWER — BUILT FOR UNDERSTANDING, IN LOGICAL BULLET POINTS, never "
                  "an essay. Answer this ONE issue as a short chain of clear bullets a student can "
                  "follow, understand, and remember:\n"
                  "- OPEN with a one-line DIRECT answer to the issue (the conclusion).\n"
                  "- Then the reasoning as SHORT BULLETS (markdown '- '), each ONE clear step. "
                  "EVERY bullet that states a rule MUST cite the specific supporting law inline — "
                  "the exact section/article/case ('under s.9(1) of Act 703…', 'per article "
                  "268(1)') — so the reader always sees WHERE the law backs the point. NEVER state "
                  "a legal proposition without naming the authority for it. Use a nested sub-bullet "
                  "to quote or paraphrase the key words of that authority when it helps.\n"
                  "- Give ONE simple, concrete EXAMPLE that makes the point click — plain and "
                  "everyday ('e.g. owning the land but holding no mineral right still can't mine').\n"
                  "- APPLY it to THIS problem's facts in a bullet or two and state the consequence "
                  "for the party.\n"
                  "Keep every bullet short and plain. The GOAL is that the reader understands the "
                  "point AND knows exactly which law supports it — not length. No intro, no "
                  "background lecture, no restating the facts, no 'further facts would sharpen "
                  "this' digression, no essay prose.")
    else:
        system = (CONFIG["system_prompt"] + "\n\n" + WRITING_STYLE + "\n\n" + DEPTH
                  + "\n\n" + ORIGINALITY + "\n\n" + LEGAL_METHOD + "\n\n"
                  + GRUNDNORM_METHOD + "\n\n" + CASE_APPLICATION + "\n\n" + FACT_DISCIPLINE + "\n\n" + REFORM_METHOD + "\n\n"
