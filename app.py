...
def answer_question(course, question, include_web=True, fmt="essay", max_out=8000,
    ...
    if mode == "cases":
        ...
        system = (CONFIG["system_prompt"] + "\n\n" + LEGAL_METHOD + "\n\n"
                  ...
                  "REPRODUCE THE LAW VERBATIM, DON'T PARAPHRASE IT: for each governing provision, set "
                  "out its OPERATIVE CONTENT as the EXACT WORDS of the provision, quoted verbatim from "
                  "the retrieved passage and placed in quotation marks, with the pinpoint. This is "
                  "mandatory — do NOT summarise, compress, modernise, re-order or 'tidy' the statutory "
                  "/ treaty wording into your own words; the words themselves are what the Application "
                  "must apply, so they must be the real ones. Quote the whole operative limb (not a "
                  "fragment that changes its sense), and quote every limb/proviso/exception the "
                  "provision contains. The ONLY time you may state terms without quotation marks is "
                  "where the retrieved passage genuinely does not reproduce the provision's words (it "
                  "only refers to or describes it) — and then say so explicitly ('the passage refers "
                  "to but does not set out s.X') rather than reconstructing the wording from memory. "
                  "Naming 's.12 of Act "
    ...
    if mode == "gather":
        ...
        rule_sys = cached_system(
            ...
            "its instrument; note the default rule then any exception, and any repeal/succession. Do "
            "NOT write an Issue, an Application or a Conclusion, and do NOT apply the law to the "
            "facts. If a governing provision's text is not actually reproduced in the passages (only "
            "referred to), say 'not in the materials — provision it' for that point and do NOT "
            "reconstruct the wording from memory. Output tight markdown bullets, each a quoted "
            "provision with its pinpoint — this is the Rule the writer will apply." + new_only)
