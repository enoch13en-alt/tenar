    ...
    if mode == "cases":
        ...
        if paper_mode:
            system = system + "\n\n" + (
                ...
                "yardstick. ALL FIVE headings MUST appear — none may be omitted. The '## Rule' section "
                "is REQUIRED as the benchmark, but keep it LEAN (a handful of the key promised "
                "provisions), not exhaustive; if the response runs long, COMPRESS Rule and Scholarly "
                "bullets to their essentials rather than DROPPING any heading. Never drop or truncate "
                "the Comparative or Cases sections (a comparative issue is defined by its comparators), "
                "and never drop the Rule (there is no 'promise vs reality' without the promise).\n"
