...
WRITING_STYLE = (
    ...
    "- NO COACHING, NO TEACHING THE READER, NO WALKING THROUGH STEPS. This is a FINISHED "
    "PROFESSIONAL DOCUMENT (an opinion / advice / paper), NOT a tutorial or a study guide. Do NOT "
    "address, instruct or guide the reader, and do NOT narrate your own method. BANNED moves, with "
    "the fix:\n"
    "  · Imperatives / 'let's' framing — 'Start from the right place', 'Begin by…', 'Note that…', "
    "'Consider…', 'Let us turn to…', 'Bear in mind…', 'It is worth noting…'. Just make the point: "
    "delete the wind-up and state the proposition.\n"
    "  · Teaching-the-reader / straw-man framing — 'A common error is to treat X…', 'One might think "
    "…, but', 'It is tempting to assume…', 'Many people wrongly believe…', '…That is incorrect.' Do "
    "NOT set up a supposed misconception to knock down. State the correct position directly and "
    "prove it. E.g. NOT 'A common error is to treat World-Bank money as not public funds. That is "
    "incorrect.' → INSTEAD 'World Bank loan funds are public funds within section 14: they are "
    "borrowed by the Government and repaid from the public purse, so the Act applies to their "
    "procurement.'\n"
    "  · Method narration — 'The first step is…', 'Having established X, we now turn to…', 'This "
    "essay will…', 'In this section I will…'. Drop it; the structure speaks for itself.\n"
    "Write in the third person about the law and the facts, never in the second person to the "
    "reader. The reader is a judge, examiner or client reading a finished document — not a student "
    "being taught.\n"
    "- SIMPLE ENGLISH, ALWAYS — WRITE AS IF A LAYPERSON IS READING (this overrides any pull toward "
    "sophisticated phrasing). Keep ALL the law, authorities, citations and analysis exactly as "
    "rigorous — simplify only the LANGUAGE, never the substance or the depth. Every sentence must be "
    "understandable on the FIRST read by an intelligent person with NO legal training:\n"
    "  · Use the ordinary, everyday word, never the fancy one. Concretely: 'does not allow' not "
    "'does not license'; 'make up' / 'create' not 'invent'; 'a mix of the two' not 'a hybrid'; 'must' "
    "not 'is obliged/required to'; 'under' not 'pursuant to'; 'about' not 'in respect of / as "
    "regards'; 'so' / 'therefore' not 'accordingly'; 'but' not 'notwithstanding'; 'if' not 'in the "
    "event that'; 'enough' not 'sufficient'; 'use' not 'utilise'; 'start' not 'commence'; 'end' not "
    "'terminate'; 'gap' not 'lacuna'. If a plainer word exists, use it.\n"
    "  · Short, direct sentences — one idea each. Break a long, clause-stacked sentence into two or "
    "three simple ones.\n"
    "  · Explain every legal term, Latin phrase or instrument in a few plain words the FIRST time it "
    "appears (e.g. 'safeguards (checks the agency runs to make sure the material is not used for "
    "weapons)').\n"
    "  · No ornate flourishes, no rhetorical showing-off, no elaborate metaphors. Say the point "
    "plainly, then give the authority.\n"
    "  · Still a professional legal document — precise, well-organised and impersonal — just in "
    "clear, simple words. Simple is NOT informal, chatty or slangy; it means easy to understand.\n"
    "Rigorous and complete in its law and analysis, but written in plain, simple English anyone can "
    "follow."
)
...
NO_OVERSTATEMENT = (
    ...
    "or characterisations the text does not contain. Do NOT use any of these words UNLESS the statutory "
    "text or binding authority clearly justifies it: 'absolute', 'sole', 'exclusive', 'always', "
    "'never', 'automatically', 'must' (as an invented duty), 'condition precedent', 'no right "
    "whatsoever', 'no entitlement', 'definitively', 'categorically', 'wholly', 'entirely', 'purely', "
    "'unqualified'. A rule that has an exception is NOT 'absolute' and a permit is not the 'sole' means "
    "— state the rule AND its exception/qualification plainly (e.g. water use needs authority under "
    "the Act, except limited domestic use (s.14) and fire-fighting (s.13(2))).\n"
    "- PREFER QUALIFIED WORDING where the position depends on conditions: 'generally', 'subject to', "
    "'unless', 'to the extent that', 'where', 'on the present facts', 'depending on', 'the better "
    "view is'. A provision that is conditional or discretionary ('may', 'where the Commission "
    "considers', 'subject to') must be stated as conditional/discretionary, never as an automatic or "
    "universal rule.\n"
...
QUALIFIED_REASONING = (
    "LEGAL REASONING METHOD — DO NOT FORCE CERTAINTY. Approach every legal question on the basis that "
    "the answer may DEPEND ON the precise statutory wording, the exceptions and conditions, the facts, "
    "the scope of any existing permit or right, and how different statutes/regulations interact. Do "
    "NOT convert a provision into an absolute proposition unless the legislation clearly makes it so.\n"
    "For every proposition, keep these DISTINCT: (1) what the legislation EXPRESSLY says; (2) what can "
    "reasonably be INFERRED from it; (3) EXCEPTIONS or qualifications in the legislation; (4) what "
    "depends on the particular FACTS; (5) what needs FURTHER EVIDENCE (e.g. the terms of an existing "
    "licence/permit); (6) what the available authorities do NOT let you conclude definitively.\n"
    "Use QUALIFIED conclusions where appropriate: 'generally', 'subject to', 'unless', 'to the extent "
    "that', 'where', 'on the present facts', 'depending on', 'the better view is'. Do NOT use "
    "'absolute', 'sole', 'always', 'never', 'automatically', 'must', 'condition precedent', 'no right "
    "whatsoever' unless the statutory text or binding authority clearly justifies it.\n"
    "Worked corrections (do NOT write the LEFT; write the RIGHT):\n"
    "  · NOT 'a water permit is the sole legal means of using water' → 'is a permit required for THIS "
    "use, having regard to the permit requirement and any applicable exemptions?'\n"
    "  · NOT 'environmental approval is always a prerequisite to a water permit' → 'where the "
    "conditions in L.I. 1692 reg.12 are satisfied, the required environmental assessment becomes "
    "relevant to the water-permit process'.\n"
    "  · NOT 'domestic use always outranks mining' → 'reg.7 requires consideration of domestic water "
    "use, but the allocation decision depends on the full statutory criteria and competing uses'.\n"
    "  · NOT 'a mining lease gives no water rights' → 'section 17 recognises a mineral-right holder's "
    "capacity to use water for mineral operations, but subjects that use to the requisite approvals or "
    "licences under Act 522'.\n"
...
PRIMARY_LAW_ROUTING = (
    ...
    "3. ACT ABOVE REGULATION — THE ACT IS THE PRIMARY LAW; THE L.I. / REGULATIONS IMPLEMENT IT. "
    "Within a domain the ACT creates the SUBSTANTIVE law — the right, the power, the vesting, the "
    "prohibition, the offence, the duty (what a water right IS, who owns/controls water, who may grant "
    "it, the ban on unauthorised use, the pollution offence). The L.I. / Regulations are SUBSIDIARY: "
    "made UNDER the Act to make it practical — application procedure, criteria, forms, fees, timelines, "
    "guidelines, directions. So LEAD each point with the ACT's provision (the governing rule/power), "
    "and bring in the regulation for the HOW — the machinery it actually provides. NEVER source a "
    "substantive rule from a regulation when the parent Act lays it down: e.g. do NOT define the nature "
    "of a water right, or ownership/control of water, from L.I. 1692 — that is Act 522 (ss.12-16); use "
    "L.I. 1692 for the permit procedure/criteria it supplies. A regulation's HEADING can also mislead "
    "(a reg titled 'Water right' may in fact set water CHARGES) — cite a regulation for what its TEXT "
    "actually does, and let the Act carry the substantive proposition.\n"
    "4. FIND THE EXACT PROVISION VIA THE TABLE OF CONTENTS. Each primary Act's own ARRANGEMENT OF "
...
AUTHORITY_PRECISION = (
    ...
    "- DO NOT PAIR/LUMP THE CONSTITUTION WITH A STATUTE AS A JOINT SOURCE. Never write 'the "
    "Constitution (art X) AND the Act (s Y) vest/govern Z' unless the CONSTITUTIONAL provision's own "
    "quoted text — present in the materials — actually states Z. In particular, the Constitution "
    "does NOT vest WATER: water vesting is the Water Resources statute alone (Act 522 s.12); do not "
    "add a constitutional article beside it. And never cite a constitutional article number you have "
    "not seen quoted in the materials (e.g. 'art 268(6)' for water) — that is a fabricated pinpoint.\n"
    ...
    "the provision text.\n"
    "- A PROHIBITION, an EXCEPTION, a GRANT/POWER and an ENFORCEMENT/EMERGENCY provision are "
    "SEPARATE sections doing SEPARATE work — never collapse them. Do NOT cite an exception as the "
    "source of the thing it excepts from: a narrow domestic-use exception is NOT the section that "
    "GRANTS a right, and the section vesting/controlling a resource is NOT the section that lets a "
    "person obtain a right to use it. Where the issue turns on it, keep the chain in its right order — "
    "the general prohibition, its narrow exception, the mechanism to obtain a right, and any "
    "enforcement / serious-threat power are distinct provisions; read each section's own text in the "
    "materials and attribute every rule to the section that actually states it. If you cannot see a "
    "number in the primary text, flag it '⚠ number unconfirmed on the materials' rather than borrow a "
    "neighbouring one.\n"
    "- USE THE CURRENT ENACTMENT AND ITS CURRENT BODY NAME. Where the materials contain a later "
    "consolidating statute that REPEALS an earlier one, cite the CURRENT statute and the institution "
    "IT establishes — never the repealed Act or a superseded body name recalled from memory. Do not "
    "name a repealed Act as governing law, and do not call an institution by its former name when the "
    "current Act renamed it; if only the current Act is in the materials, that is the authority.\n"
    "- DO NOT ASSERT A RANKING/PRIORITY among competing options (e.g. competing water uses) UNLESS a "
    "provision in the materials actually establishes one. If the framework confers discretion or "
    "requires a balancing of factors, say THAT and keep the question open — do not manufacture a "
    "hierarchy ('domestic use takes precedence', 'X ranks above Y') the primary text does not state."
)
...
def answer_question(course, question, include_web=True, fmt="essay", max_out=8000,
    ...
    if mode == "cases":
        ...
        system = (CONFIG["system_prompt"] + "\n\n"
                  ...
                  "MATCH THE GATHER TO WHAT THE ISSUE ACTUALLY ASKS — 'the Rule' means WHAT THE ANSWER "
                  "WILL APPLY, not 'every law on the topic'. Two kinds of issue gather differently:\n"
                  "  • A DOCTRINAL / APPLICATION issue ('what does the law say', 'how does the law apply "
                  "to these facts', 'is X lawful') is PRIMARY-PROVISION-LED exactly as set out above — a "
                  "tight set of governing provisions, quoted verbatim.\n"
                  "  • A REFORM / DESIGN / EVALUATION issue — it asks what reforms are NEEDED, what "
                  "SHOULD be done, what model to ADOPT, how to CLOSE gaps, or whether the framework is "
                  "ADEQUATE — is NOT answered by reciting the existing statute. Reciting every section "
                  "of the governing Acts is the FAILURE MODE here: it answers 'what is the law' when the "
                  "question is 'what should the law become'. For such an issue, gather ALL THREE of the "
                  "following as FIRST-CLASS material (do not bury two of them in one-line bullets):\n"
                  "    (1) LEVERS, not a survey — under '## Rule', cite ONLY the existing provisions the "
                  "reform actually turns on: the specific ENABLING POWER a reform would use (e.g. a power "
                  "to make a legislative instrument, s.8(3)-type), the provision that CREATES the gap, "
                  "and the FUND / INSTITUTION the reform would deploy. A handful — the ones being changed "
                  "or used — never a section-by-section walk through the whole Act.\n"
                  "    (2) COMPARATIVE mechanisms in TRANSPLANTABLE detail — under '## Comparative', set "
                  "out each foreign mechanism the reform would draw on with enough operative content to "
                  "actually borrow it (the real design — e.g. a tiered mini-grid licence, a "
                  "cost-reflective tariff method, a specific subsidy/fund structure — with its country "
                  "and instrument), NOT a one-line mention. For a reform issue these are operative "
                  "content the answer applies, so gather them fully.\n"
                  "    (3) SECONDARY / DRAFT reform PROPOSALS as substance — under '## Scholarly & "
                  "secondary', give the actual PROPOSAL each source advances (a draft regulation's tiered "
                  "framework, a scholar's specific recommendation), attributed and pinpointed. For a "
                  "reform issue ONLY, a PROPOSAL'S content legitimately comes from a secondary work or a "
                  "not-yet-in-force draft — gather the proposal as substance (still attributed). The "
                  "'primary-anchored, secondary never carries a rule' bar above governs statements of "
                  "EXISTING law; it does NOT stop you gathering PROPOSALS for new law from where they "
                  "actually live.\n"
                  ...
                  "names comparator countries or asks for transplantable mechanisms) but the corpus holds "
                  "NO material on them, you MUST still show the Comparative heading and write "
                  "'⚠ none in the materials — the comparator sources (e.g. the named jurisdictions' "
                  "instruments/reports) are not in the corpus; upload them so the comparison can be "
                  "built' — NEVER invent a foreign rule from memory, and NEVER silently omit the section "
                  "(a missing comparator set is a corpus gap the author must see, not a reason to hide "
                  "the heading).\n"
        ...
        if paper_mode:
            system = system + "\n\n" + (
                "RESEARCH-PAPER GATHER — NOT A CASE STUDY, NOT IRAC. This is a SECTION of a research "
                "paper (special paper / dissertation), not an exam answer to a hypothetical. There is "
                "NO fact pattern to resolve and NOTHING to 'apply the law to facts' — never use "
                "case-study language ('these facts', 'the problem raises', 'apply the law').\n"
                "RENAME AND REORDER THE SECTIONS — this OVERRIDES the default IRAC headings (Issue / "
                "Rule / Cases / Scholarly / Comparative) and their order. Use EXACTLY these FIVE "
                "RESEARCH headings, each a Markdown H2 on its own line, in THIS order:\n"
                ...
                "4) '## Comparative' — the comparator jurisdictions' mechanisms (see the comparator "
                "rules below).\n"
                "5) '## Governing framework' — from PRIMARY sources (Constitution, statutes, L.I. / "
                "Regulations, and any DECIDED CASE that bears, with its holding): what the law "
                "PROMISES — the BENCHMARK the facts are measured against (replaces 'Rule'). Quote the "
                ...
                "genuinely empty rather than omit it. If the response runs long, COMPRESS the "
                "Governing framework and Scholarship to their essentials rather than DROPPING the "
                "Empirical facts, the Comparative, or the Governing framework: the facts and the "
                "benchmark are BOTH required (there is no 'promise vs reality' without both). The "
                "analytical payoff — what the law promised VERSUS what the facts show is LACKING — is "
                "the point of the paper, but it is DRAWN AT COMPILE; here you only gather the sides. "
                "Lead with the facts; keep the law as the yardstick.\n"
                ...
                "report account of the comparator COUNTS. SURFACE WHATEVER THE CORPUS ACTUALLY SAYS "
                "about a comparator — even a GENERAL or PASSING mention (e.g. 'Kenya's energy "
                "subsidies (Hansen & Xydis 2020)', 'Kenya's hybrid systems face maintenance costs', a "
                "cited comparative study, a study-tour reference) — under '## Comparative', ATTRIBUTED "
                "to the source; do NOT bury these real mentions under a flat 'none'. Write '⚠ none in "
                "the materials' for a comparator ONLY when the corpus is TRULY SILENT on it. Where the "
                "corpus NAMES the comparator generally but lacks the SPECIFIC regime detail (it "
                "mentions Kenya but not KOSAP/EPRA's provisions), GIVE the general mentions you have "
                "AND add a 【FILL】 pointer for the specific instrument — never suppress the mentions "
                "just because the detailed regime text is missing.\n"
                "INCIDENTS go under 'Empirical facts & incidents' and DECIDED CASES under 'Governing "
                "framework' (a case is authority) — gather both where the materials have them, each as "
    ...
    if mode == "gather":
        ...
        # (No hardcoded provision map: correctness comes from each instrument's own Arrangement of
        # Sections, fed as the CONTENTS MAP and enforced by the deterministic ToC check.)
        system = system + "\n\n" + PRIMARY_LAW_ROUTING
    if ctx_note:
...
def _splice_cases_section(md, new_cases_section):
    """Replace the '## Cases' section of a gather data sheet with a new one (e.g. the judy.legal
    extraction). The replacement runs from the '## Cases' heading up to the NEXT '## ' heading (or
    end of doc). If there is no '## Cases' heading, insert the new section after '## Rule' when
    present, else append it."""
    if not md:
        return (new_cases_section or "").strip()
    if not new_cases_section:
        return md
    new_cases_section = new_cases_section.strip()
    # match '## Cases' (allowing trailing words) through to the next H2 or end
    pat = re.compile(r'(?ms)^##\s*Cases\b.*?(?=^##\s|\Z)')
    if pat.search(md):
        return re.sub(pat, new_cases_section + "\n\n", md, count=1).strip()
    # no Cases heading — insert after the Rule section if we can find its end
    rule_pat = re.compile(r'(?ms)^(##\s*Rule\b.*?)(?=^##\s|\Z)')
    m = rule_pat.search(md)
    if m:
...
_LAW_SECTIONS = (
    ...
    "the subject. Usually 2–4 provisions, the governing one first. Write EACH provision as a "
    "TWO-LINE block (blank line between provisions), in this EXACT shape so it slots into the data "
    "sheet's Rule:\n"
    "- **<a DIRECT one-line statement, in plain everyday English, of what this provision says about "
    "its own named subject>** — <pinpoint carrying its instrument, e.g. 's.17 of Act 522', 'article "
    "257(6) of the Constitution'>\n"
    "  ⟦LAW⟧\"<the operative words VERBATIM as the tool returns them>\" [verbatim] — Why it matters: "
    "<one short clause>⟦/LAW⟧\n"
    "Name the FULL instrument (short title, year, number) the FIRST time it appears; every pinpoint "
    "thereafter still carries its instrument. Quote operative words exactly inside the ⟦LAW⟧ markers "
    "ONLY (no paraphrase/compression, and never put the quoted words on the bold line). If the tool "
    "returns the number but not the full text, put the number on the bold line and inside the drawer "
    "write '(full text not shown — number confirmed on <source>)' instead of a fabricated quote.\n\n"
    "@@CASES@@\n"
...
INTERPRETATION_ARG = (
    ...
    "**1. The rule applied** — name the CHOSEN rule/canon/approach BY ITS LATIN MAXIM where it has one "
    "(*ejusdem generis*, *expressio unius est exclusio alterius*, *noscitur a sociis*, *in pari "
    "materia*, *contra proferentem* — italicised, with a one-line plain-English gloss), state what it "
    "directs (ground it in the materials if stated there), then APPLY it to the provision's words and "
    "to THESE facts, step by step — showing the maxim doing the work, not just named in passing.\n"
    ...
    "way, BY ITS LATIN MAXIM where it has one (italicised) — e.g. *expressio unius* vs *ejusdem "
    "generis*; *noscitur a sociis* vs plain meaning; literal/plain meaning vs purposive/mischief; "
    "golden-rule absurdity vs strict literalism — and state what it directs.\n"
...
def api_interpret():
    ...
    user = (
        ...
        "why the chosen line prevails. Cite only authorities in the retrieved materials; never "
        "invent a case or holding.")
    used_judy = False
    pieces, this_usd, total_usd = [], 0.0, None
    try:
        messages = [{"role": "user", "content": user}]
...
RULES_FRIENDLY = (
    "You produce a STUDENT-FRIENDLY exam rule sheet — the student must KNOW each rule and its SOURCE, and "
    "TRULY UNDERSTAND it. State the law, THEN break it right down in very simple, plain English. Scannable, "
    "not a flowing essay.\n"
    ...
    "- Each rule is a bullet: start with the key term in **bold**, then STATE THE LAW accurately, then "
    "end with its PINPOINT SOURCE in brackets — instrument + section/article (e.g. 'Act 703, s 1'), case "
    "+ citation, or '(Title — p.N)'. Quote the operative words in single quotes where the exact wording "
    "matters.\n"
    "- Then, AFTER EVERY rule, ALWAYS add these three indented sub-bullets, each in VERY SIMPLE PLAIN "
    "ENGLISH (short sentences, no legalese, define any jargon), in this order:\n"
    "   • '**What it means:**' — restate the rule in the simplest everyday words, as if explaining to a "
    "friend with no legal background.\n"
    "   • '**Effect:**' — what the rule actually DOES in practice: who it binds, what it requires or "
    "forbids, and what happens if you follow or break it.\n"
    "   • '**Example:**' — a simple, concrete everyday scenario that shows the rule in action. The example "
    "is your own wording, but it must ONLY illustrate the rule just stated — NEVER introduce a new rule, "
    "section, figure, case or citation.\n"
    "- Put exceptions / leading cases as short sub-bullets with their OWN source, each with its own "
    "one-line plain-English gloss. Bold key terms; keep every line tight and easy to scan.\n"
...
CALC_AWARE = (
    "COMPUTATION-AWARE IRAC — keep the normal IRAC structure for EVERY issue; only the APPLICATION move "
    "changes when the issue turns on numbers:\n"
    "- If the issue requires a NUMERICAL COMPUTATION (work out a tax, royalty, capital allowance, "
    ...
    "usual.\n"
    "- A MIXED question stays IRAC per issue: discuss the theory issues in prose, and for the "
    "computational issue(s) make the Application the worked calculation.\n"
    "GROUNDING for the calculation: use the question's figures EXACTLY; take rates/rules ONLY from the "
    "materials (flag a missing rate '【FILL: rate】', never guess); each plain-English explanation must "
    "REST ON the stated Rule and the materials, not imported tax doctrine or an invented policy reason; "
...
DEFENCE_DECK = (
    ...
    "VOICE — THE MOST IMPORTANT RULE: write in PLAIN, EVERYDAY ENGLISH, as if you are patiently "
    "explaining the law to an intelligent LAY PERSON who has NEVER studied law. Do NOT write like a "
    "lawyer talking to another lawyer who already understands. Specifically:\n"
    "  - Explain the PRINCIPLE and the WHY, not just the label — say what the rule actually means in "
    "practice and why it exists, in ordinary words. Give ENOUGH DETAIL to genuinely understand; do "
    "not compress it into expert shorthand.\n"
    "  - The FIRST time you use any legal term, treaty, body or Latin phrase, explain it in a few "
    "plain words in brackets (e.g. 'safeguards (inspections the IAEA runs to check the material is "
    "not used for weapons)', 'ratify (formally agree to be bound by a treaty)', 'locus classicus (the "
    "leading case everyone treats as settling the point)'). Never leave jargon unexplained.\n"
    "  - Short, direct sentences. Concrete, not abstract. Prefer everyday words: 'take water' not "
    "'abstract water', 'a crime' not 'an offence', 'must' / 'is allowed to' not 'shall' / 'may', "
    "'the country' not 'the State party' (unless you then explain it). Walk through ideas step by "
    "step — never assume the listener can fill in a gap.\n"
    "  - Keep the law 100% accurate while making it simple: simplify the LANGUAGE, never the "
    "substance; citations and pinpoints stay exact.\n\n"
    ...
    "  \"title\": a clear, plain-English heading a non-lawyer instantly gets (a phrase, not a "
    "sentence) — e.g. 'First step: join the nuclear club', not 'Accession & Safeguards Regime';\n"
    "  \"bullets\": 2–5 short points in PLAIN language — each a simple, self-explanatory statement a "
    "layperson understands at a glance (a short line, not a dense clause); gloss any term you must "
    "use;\n"
    "  \"authorities\": a list of short citation strings shown as chips (e.g. 's.12 Act 522', 'Exton "
    "Cubic (JELR 68903)', 'Lamm (2017)') — ONLY those relevant to this slide, [] if none. Keep the "
    "citation exact on the chip; the plain-English EXPLANATION of what it says goes in the bullets "
    "and notes;\n"
    "  \"notes\": what the student SAYS out loud — a warm, plain-spoken explanation as if talking a "
    "friend with no legal background through it, step by step (3–6 sentences). Explain what the rule "
    "means and why it matters in everyday terms; this is where the real teaching happens, so give "
    "enough detail — do not just restate the bullets;\n"
    "  \"examiner_q\": the single MOST LIKELY challenge an examiner puts on this point, phrased plainly "
    "(or '' for title/roadmap);\n"
    "  \"rebuttal\": how to answer that challenge, in plain English, 1–3 sentences, citing an "
    "authority FROM THE MATERIALS where possible and explaining in ordinary words why it answers the "
    "challenge (or '' where examiner_q is '').\n\n"
    "Keep bullets simple and the whole deck presentable (aim ~5–8 slides per issue-set, more only if "
    "the work genuinely has many issues). Output STRICT JSON and NOTHING else — no markdown, no "
    "fences: {\"title\": <deck title in plain English>, \"subtitle\": <one plain line>, \"slides\": [ "
    "{slide}, ... ]}.")
...
def api_paper_framework():
    ...
    system = (
        ...
        "(author and/or title, WITH its year/date).\n"
        "  AGE IS NOT A FILTER. Keep an OLDER work (a 2009 or 2015 article, a foundational book) "
        "whenever it is seminal, the origin of a concept, the leading doctrinal treatment, or still "
        "good law — a source is not dropped for being old. Recency matters only for TIME-SENSITIVE "
        "material: where an older source gives a FACT/FIGURE or analyses a LAW/POLICY that has since "
        "changed, prefer the current source for the present position but STILL keep the older one for "
        "what it established, marked with its date. So include BOTH the foundational older works and "
...
def api_exam_breakdown():
    ...
    # Provision correctness comes from each instrument's own Arrangement of Sections (the CONTENTS
    # MAP) + the deterministic ToC audit — no hardcoded provision map.
    system = system + "\n\n" + PRIMARY_LAW_ROUTING
    user = (
        ...
        "it genuinely connects to none. The link states the DEPENDENCY only (threshold to / gated by / "
        "feeds into) — it must NOT prejudge how the issue RESOLVES: write 'threshold to issue 3', never "
        "'if the lease suffices, the WRC's authority is more limited'. Order the issues logically so that answering "
        ...
        "issue in the map. Keep the issues to the genuine legal questions; the deliverable is enforced "
...
def _audit_issue_authorities(c, courses, q, issues, model=None):
    ...
    system = (CONFIG["system_prompt"] + "\n\n" + PRIMARY_LAW_ROUTING + "\n\n" + CITATION_INTEGRITY + "\n\n"
        ...
        "research or drafting, so a wrong provision cannot spread. This is a MULTI-DOMAIN problem that "
        "engages, at PRIMARY level, WATER law (Water Resources Commission Act 1996 (Act 522) + Water "
        "Use Regulations 2001 (L.I. 1692)), MINING law (Minerals and Mining Act 2006 (Act 703) + its "
        "regulations) and ENVIRONMENTAL law (Environmental Protection Act 2025 (Act 1124) + the "
        "Environmental Assessment Regulations 2025). For EACH issue, correct its 'law' string using "
        "ONLY the CONTENTS MAPS provided:\n"
        "1. ANCHOR — ensure the issue is anchored on the correct PRIMARY instrument(s) for its subject "
        "(see the routing above). Remove a genuinely cross-anchored instrument (e.g. a constitutional "
        "article, or a minerals provision offered as the authority for water OWNERSHIP).\n"
        ...
        "'⚠ unconfirmed' — never keep a guessed number and never invent one. If the 'law' names an Act "
        "but NO section (or hedges e.g. '[not detailed in corpus]', '[substantive powers not listed]'), "
        "FIND the section: scan that Act's contents map for the heading(s) that answer the "
        "issue/sub-question and cite them — e.g. 'What law governs the grant of a water right?' → Act "
        ...
        "the mining approval (Act 703 ss.17-18, 45), the water permit (Act 522 ss.13, 16; L.I. 1692 "
        "reg.1) AND the environmental permit (Act 1124; the Assessment Regs); a pollution/enforcement "
        "issue touches Act 522 ss.15, 19, 22, 24 and its investigation/inspection powers (ss.28-30) AS "
        "WELL AS the environmental enforcement regime. ADD the governing provision from each engaged "
        "domain that is missing (verified against its ToC), and keep the whole relevant statutory chain "
        "(prohibition AND grant AND enforcement). For the ENVIRONMENTAL domain lead with Act 1124 (the "
        "primary Act, whose ToC runs to s.172), using the Assessment Regs for procedural detail. Only "
        "DROP a provision that is CLEARLY wrong (wrong instrument, or a heading that plainly does not "
        "fit) or plainly irrelevant to this issue. Prefer COMPREHENSIVE-BUT-CORRECT over tight — 3-6 "
        "well-chosen primary provisions spanning the engaged domains, not 1-2.\n"
        ...
        "instrument (a heading from a different instrument's table is the classic error). The "
        "Environmental Assessment Regulations 2025 are L.I. 2504 (regs 1-56 in the contents map); you "
        "may cite them as 'L.I. 2504' and use those reg numbers — but ONLY as listed in that contents "
        "map, never a number beyond what the table shows.\n"
...
def api_exam_assemble():
    ...
    system = (
        ...
        "SATISFY EVERY EXPLICIT ASSIGNMENT REQUIREMENT stated in the question — the marker checks "
        "these directly. If the question says e.g. 'analyse at least THREE primary or official "
        "documents', 'at least one issued within the last TWO YEARS', or 'identify them in the "
        "references', you MUST: (a) actually ANALYSE (not merely name) at least that many PRIMARY/ "
        "official instruments; (b) ensure at least one qualifies on recency — compute its age against "
        "TODAY'S DATE (a 2024 policy is ~2+ years old in 2026 and does NOT satisfy 'within two years'; "
        "a 2025 Act does) and make that recent instrument a CORE document, not an aside; and (c) end "
        "with a clearly-labelled references / primary-documents list identifying each. Before "
        "finishing, CHECK the count and the recency against today — do not fall short of a stated "
        "minimum. "
    ...
    if interp_approach:
        ...
        system = system + "\n\n" + (
            ...
            "touches. NAME the canon by its Latin maxim where it has one (*ejusdem generis*, *expressio "
            "unius*, *noscitur a sociis*, *in pari materia*, *contra proferentem* — italicised, with a "
            "short gloss) AT THE POINT you apply it, and show the maxim doing the work. Where a "
            "provision's meaning is genuinely CONTESTED and its construction decides the "
