...
FACT_DISCIPLINE = (
    ...
    "don't hedge it in. A padded, unraised issue is a deduction, not a bonus.\n"
    ...
    "- ADDITIONAL FACTS ONLY WHERE GENUINELY OMITTED AND OUTCOME-CHANGING. Flag a missing fact "
    "ONLY where BOTH (1) the examiner has genuinely left a MATERIAL fact silent, AND (2) the "
    "legal answer CHANGES depending on it — never merely because more information would be "
    "useful. When both are met, put it under the heading 'Additional Facts Material to the "
    "Advice' and give three things: the missing fact; why it matters legally; how the advice "
    "would change. Only there do you analyse the alternatives, and only because the omission "
    "genuinely changes the outcome ('the facts do not state whether the ancestral forest is "
    "gazetted; if it is, additional statutory restrictions apply'). A fact the examiner HAS "
    "stated is never an 'additional fact', and real-world legal background (a real institution "
    "exists, a real treaty is in force, lithium is a mineral) is stated as established law, not "
    "flagged for confirmation. Genuine uncertainty in the LAW is handled by resolving it or "
    "bracketing it so the argument still proceeds (see the doctrinal register), never by a "
    "verification note.\n"
...
DOCTRINAL_PRECISION = (
    ...
    "that brackets the uncertainty and carries the reasoning forward.\n"
...
PRECISION_DISCIPLINE = (
    ...
    "level; a genuine verification flag, if truly essential, belongs in ONE short "
    "closing note, never scattered through the argument.\n"
...
def answer_question(course, question, include_web=True, fmt="essay", max_out=8000,
    ...
    if mode == "cases":
        ...
        system = (CONFIG["system_prompt"] + "\n\n" + LEGAL_METHOD + "\n\n"
                  ...
                  "**Rule** — the governing law, stated directly, as tight bullets (markdown '- '). "
                  "EVERY rule names its authority: the FIRST time an instrument appears, name it IN "
                  "FULL — short title, year and number ('the Minerals and Mining Act, 2006 (Act "
                  "703)', 'the 1992 Constitution', 'the Companies Act, 2019 (Act 992)') — and every "
                  "pinpoint thereafter CARRIES that instrument: write 's.9(1) of Act 703', 'article "
                  "268(1) of the Constitution', NEVER a bare 's.9' or 'section 23' with no "
                  "instrument attached. If a cited provision points to a repealed enactment, name "
                  "its current successor in full (see the succession rule). State the default rule "
                  "first, then any exception. Never assert a proposition without its authority.\n"
                  "**Application** — apply those rules to THIS problem's facts, directly: take the "
                  "actual parties and events and run them through the rule, step by step, to the "
                  "result. This is where you reason — not recite. Distinguish materially different "
                  ...
                  "concrete consequence for the party. Where a single further fact would flip the "
                  "outcome, name it in a final short clause; otherwise stop.\n"
...
OUTLINE_COVERAGE = (
    ...
    "record whether it was NAMED in the outline or INFERRED as the governing instrument for "
    "a topic, and which topic it belongs to. Include a required reading ONLY if it is itself "
    "a source of law (an instrument or a case), not a textbook, chapter, article or lecture "
    "note. De-duplicate the same instrument across topics.\n"
    ...
    "\"basis\":\"named|inferred\"}], \"missing\":[{\"name\":..., \"topic\":<lecture/topic>, "
    "\"basis\":\"named|inferred\", \"provisions\":<key provisions the topic leans on, if "
    "identifiable>, \"load_bearing\":\"high|medium|low\", \"why_absent\":<one line — what "
    "the corpus has instead, or nothing>, \"search_query\":<a precise query that would "
    "locate its authoritative published text>}]}. 'present' = required by the outline AND "
    "held as text; 'missing' = required by the outline but NOT held as text. Rank 'missing' "
    "high→low by how central the instrument is to the course."
)
