...
CALC_AWARE = (
    ...
    "sentences) — the Why must REST ON the stated rule/source and the course materials; do NOT import "
    "tax doctrine or a policy rationale the materials don't support (if the materials give no reason, "
    "simply explain what the rule does, rather than inventing why). Then a clear **Final Answer** with "
    "units (GH₵). Use the question's figures EXACTLY; take rates/rules ONLY from the materials — never "
    "invent one (flag a missing rate '【FILL: rate】'); double-check every calculation before carrying it "
    "forward.\n"
